...
def mousePress(button):
    ...
    Expects strings: 'left', 'middle', or 'right'.
    """
    if button == 'left':
...
def mouseHold(button):
    """
    Press mouse button and hold. 
    Expects strings: 'left', 'middle', or 'right'.
    """
    if button == 'right':
        win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTDOWN,0,0)
        ...
        print 'Invalid. Accepted arguments: \'left\', \'right\'.'
...
def mouseScroll(direction, amount = 0):
    """
    Expects string arguments: 'forward', 'reverse', and 'amount' (as integer).
    """
    if direction == 'forward':
        ...
        print ('Invalid argument. Requires direction of scroll (\'forward\' or '
               + '\'reverse\') as argument.')


def mousePos(cord=(0,0)):
    '''
    Sets mouse position. Expects tuple coordinate. (x,y)
    '''
...
def get_mouse():
    '''
    prints current mouse coordinates as tuple. Useful for quickly getting
    coordinates for use with mousePos()
    '''
...
def paste():
    win32api.keybd_event(0x11, 0,0,0) #ctrl
    win32api.keybd_event(0x56, 0,0,0) #C
    time.sleep(.05)
    win32api.keybd_event(0x11,0 ,win32con.KEYEVENTF_KEYUP ,0)
...
##    time.sleep(4)
    for i in string:
        if i == ' ':
            ...
            win32api.keybd_event(VK_CODE['left shift'], 0,0,0)
            win32api.keybd_event(VK_CODE['1'], 0,0,0)
            time.sleep(.05)
            win32api.keybd_event(VK_CODE['left shift'],0 ,win32con.KEYEVENTF_KEYUP ,0)
